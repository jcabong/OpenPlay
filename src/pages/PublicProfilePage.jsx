...
export default function PublicProfilePage() {
  // Route can be /user/:username OR /profile/:userId — handle both
  const { username: usernameParam, userId: userIdParam } = useParams()
  ...
  useEffect(() => {
    if (usernameParam || userIdParam) fetchData()
    ...
    <div className="min-h-screen bg-ink-900 text-ink-50 pb-24">
      ...
      <div className="px-5 pt-8">
        ...
        <div className="flex flex-col items-center mb-8">
          <div className="relative mb-4">
            {hasAvatar ? (
              <img
                ...
                className="w-24 h-24 rounded-[2rem] object-cover border-2 border-accent/30"
              />
            ) : (
          ...
          <p className="text-accent text-xs font-bold mt-0.5">@{profile?.username}</p>
          {(profile?.city || profile?.region) && (
            <div className="flex items-center gap-1 text-ink-500 text-xs mt-1.5">
              <MapPin size={10} className="text-accent" />
              {[profile?.city, profile?.region].filter(Boolean).join(', ')}
            </div>
          )}
          {profile?.bio && (
            <p className="text-ink-400 text-sm mt-3 leading-relaxed text-center max-w-xs">{profile.bio}</p>
          )}
              ...
              {Object.entries(sportBreakdown)
                ...
                .map(([sport, count]) => {
                  ...
                  return (
                    ...
                    >
                      {sportInfo && <span>{sportInfo.emoji}</span>}
