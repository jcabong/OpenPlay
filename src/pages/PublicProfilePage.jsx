...
export default function PublicProfilePage() {
  ...
  const [h2h, setH2h]             = useState(null) // head-to-head vs current user
  ...
  const load = useCallback(async () => {
    ...
    const [{ data: g }, { data: p }] = await Promise.all([
      // ✅ Filter soft-deleted games
      supabase.from('games')
        .select('*')
        .eq('user_id', usr.id)
        .eq('is_deleted', false)
        .order('created_at', { ascending: false }),
      // ✅ Use author_id so posts show correctly
      supabase.from('posts')
    ...
    // ✅ Head-to-head: only if viewing someone else's profile
    ...
    <div className="min-h-screen bg-ink-900 text-ink-50 pb-28">
      ...
      {activeTab === 'Posts' && (
        <div className="px-4 space-y-4">
          {posts.length === 0
            ? <div className="text-center py-14 text-ink-600 text-xs font-black uppercase tracking-widest">No posts yet</div>
            : posts.map(post => <PostCard key={post.id} post={post} onRefresh={() => {}} />)
          }
        </div>
      )}
