...
// ── Tier config ──────────────────────────────────────────────────────────────
const TIERS = {
  Bronze:   { color: '#cd7f32', bg: 'rgba(205,127,50,0.15)',  border: 'rgba(205,127,50,0.35)',  emoji: '🥉' },
  Silver:   { color: '#a0aec0', bg: 'rgba(160,174,192,0.15)', border: 'rgba(160,174,192,0.35)', emoji: '🥈' },
  Gold:     { color: '#f6e05e', bg: 'rgba(246,224,94,0.15)',  border: 'rgba(246,224,94,0.35)',  emoji: '🥇' },
  Platinum: { color: '#76e4f7', bg: 'rgba(118,228,247,0.15)', border: 'rgba(118,228,247,0.35)', emoji: '💎' },
  Diamond:  { color: '#90cdf4', bg: 'rgba(144,205,244,0.15)', border: 'rgba(144,205,244,0.35)', emoji: '💠' },
  Legend:   { color: '#c8ff00', bg: 'rgba(200,255,0,0.15)',   border: 'rgba(200,255,0,0.35)',   emoji: '👑' },
}
...
function TierBadge({ tier, small = false }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-0.5 font-black uppercase rounded-lg border ${
        small ? 'text-[8px] px-1.5 py-0.5' : 'text-[9px] px-2 py-0.5'
      }`}
...
function Avatar({ user, size = 'md', glowColor }) {
  ...
  const style = glowColor
    ? { boxShadow: `0 0 16px ${glowColor}40`, border: `2px solid ${glowColor}60` }
    : {}
  if (hasAvatar) {
    return (
...
function Podium({ board }) {
  ...
  const cfgFirst  = TIERS[first?.skill_tier]  || TIERS.Bronze
  const cfgSecond = TIERS[second?.skill_tier] || TIERS.Bronze
  const cfgThird  = TIERS[third?.skill_tier]  || TIERS.Bronze
    ...
    <div className="flex items-end gap-2 mb-6 px-2">
      ...
      {second ? (
        <div className="flex-1 flex flex-col items-center gap-2">
          <Avatar user={second} size="md" glowColor={cfgSecond.color} />
          <div className="text-center">
            <p className="text-[9px] font-black text-ink-400 truncate max-w-[80px]">@{second.username}</p>
            <p className="font-display font-bold text-xl leading-none" style={{ color: cfgSecond.color }}>
              {second.elo_rating}
            </p>
          ...
          <div className="w-full h-14 rounded-t-xl flex items-center justify-center"
            style={{ background: cfgSecond.bg, border: `1px solid ${cfgSecond.border}` }}>
            <span className="font-display font-bold text-2xl" style={{ color: cfgSecond.color }}>2</span>
          </div>
        </div>
      ...
      <div className="flex-1 flex flex-col items-center gap-2">
        ...
        <Avatar user={first} size="lg" glowColor={cfgFirst.color} />
        <div className="text-center">
          <p className="text-[9px] font-black truncate max-w-[80px]" style={{ color: cfgFirst.color }}>
            @{first.username}
          </p>
          <p className="font-display font-bold text-2xl leading-none" style={{ color: cfgFirst.color }}>
            {first.elo_rating}
          </p>
        ...
        <div className="w-full h-20 rounded-t-xl flex items-center justify-center"
          style={{ background: cfgFirst.bg, border: `1px solid ${cfgFirst.border}` }}>
          <span className="font-display font-bold text-2xl" style={{ color: cfgFirst.color }}>1</span>
        </div>
      </div>
      ...
      {third ? (
        <div className="flex-1 flex flex-col items-center gap-2">
          <Avatar user={third} size="md" glowColor={cfgThird.color} />
          <div className="text-center">
            <p className="text-[9px] font-black text-ink-400 truncate max-w-[80px]">@{third.username}</p>
            <p className="font-display font-bold text-xl leading-none" style={{ color: cfgThird.color }}>
              {third.elo_rating}
            </p>
          ...
          <div className="w-full h-10 rounded-t-xl flex items-center justify-center"
            style={{ background: cfgThird.bg, border: `1px solid ${cfgThird.border}` }}>
            <span className="font-display font-bold text-xl" style={{ color: cfgThird.color }}>3</span>
          </div>
        </div>
      ) : <div className="flex-1" />}
    </div>
...
export default function LeaderboardPage() {
  ...
  // ─────────────────────────────────────────────────────────────────────────
  // Fetch scope options from games.court_name / games.city / games.province
  // IMPORTANT: We use the match location stored on the game row — NOT the
  // user's profile city.  A player based in Manila may play in Pasig; the
  // leaderboard shows them under Pasig, not Manila.
  // ─────────────────────────────────────────────────────────────────────────
  const fetchOptions = useCallback(async () => {
    const { data } = await supabase
      ...
      .eq('sport', sport)
      .eq('is_deleted', false)
      .not('tagged_opponent_id', 'is', null) // only ranked (tagged) matches

    if (!data) return

    const courts    = [...new Set(data.map(g => g.court_name).filter(v => v?.trim()))].sort()
    const cities    = [...new Set(data.map(g => g.city).filter(v => v?.trim()))].sort()
    const provinces = [...new Set(data.map(g => g.province).filter(v => v?.trim()))].sort()
  ...
  // ─────────────────────────────────────────────────────────────────────────
  // Fetch leaderboard board
  //
  // For court / city / province tiers:
  //   Step 1 → query games table to find which user_ids played at that location
  //   Step 2 → fetch those users' ELO from player_elo
  //
  // This is the KEY fix.  The old code filtered player_elo rows by
  // r.user?.city which reads the user's PROFILE city — always "Manila" for
  // jcva07 regardless of where the match was played.
  // ─────────────────────────────────────────────────────────────────────────
  const fetchBoard = useCallback(async () => {
    ...
    setLoading(true)
    try {
      let userIds = null // null → no filter (national / global shows everyone)

      if (tier === 'court' || tier === 'city' || tier === 'province') {
        // Build location filter against games table
        ...
        if (tier === 'court')    locQuery = locQuery.eq('court_name', selectedCourt)
        if (tier === 'city')     locQuery = locQuery.eq('city',       selectedCity)
        if (tier === 'province') locQuery = locQuery.eq('province',   selectedProvince)

        const { data: locData, error: locErr } = await locQuery
        if (locErr) {
          console.error('Location filter error:', locErr.message)
          setBoard([]); setLoading(false); return
        }

        userIds = [...new Set((locData || []).map(r => r.user_id))]
        if (!userIds.length) { setBoard([]); setLoading(false); return }
      }

      // Fetch ELO rows — filter by user IDs when scoped
      let eloQuery = supabase
        .from('player_elo')
        .select(`
          user_id, sport, elo_rating, wins, losses, matches_played, skill_tier,
          user:users!player_elo_user_id_fkey(id, username, avatar_url, avatar_type, city, province)
        `)
        .eq('sport', sport)
      ...
      if (userIds !== null) {
        eloQuery = eloQuery.in('user_id', userIds)
      }

      const { data, error } = await eloQuery
      if (error) {
  ...
  const TierIcon   = TIER_TABS.find(t => t.id === tier)?.icon || Trophy
    ...
    <div className="min-h-screen bg-ink-900 text-ink-50 pb-24">
      ...
      <div className="px-5 pt-14 pb-4">
        ...
        <p className="text-accent text-[9px] font-black uppercase tracking-widest">
          ELO · Per Sport · Per Location · Live
        </p>
      </div>
        ...
        {SPORTS.map(s => (
          <button
            ...
            className={`flex items-center gap-1.5 px-4 py-2 rounded-xl text-[10px] font-black uppercase border shrink-0 transition-all duration-200 ${
              sport === s.id
                ? 'bg-accent text-ink-900 border-accent scale-105'
      ...
      {tier === 'court' && (
        courtOptions.length === 0
          ? <p className="text-ink-600 text-xs font-bold text-center py-2 px-5">
              No courts yet — log a tagged match at a court first!
            </p>
      ...
      {tier === 'city' && (
        cityOptions.length === 0
          ? <p className="text-ink-600 text-xs font-bold text-center py-2 px-5">
              No cities recorded — make sure to pick a court location when logging matches!
            </p>
      ...
      {tier === 'province' && (
        provinceOptions.length === 0
          ? <p className="text-ink-600 text-xs font-bold text-center py-2 px-5">
              No provinces recorded — make sure to pick a court location when logging matches!
            </p>
      ...
      {loading ? (
        ...
        <div className="text-center py-16 px-6">
          ...
          <p className="text-ink-600 text-xs mt-2 max-w-xs mx-auto">
            {tier === 'court'
              ? 'Log a tagged match at this court to appear here!'
              : tier === 'city'
              ? `No tagged matches recorded in ${selectedCity} yet.`
              : tier === 'province'
              ? `No tagged matches recorded in ${selectedProvince} yet.`
              : 'Log a match with a tagged opponent to earn an ELO rating!'}
          </p>
        </div>
      ) : (
                ...
                return (
                  ...
                  >
                    ...
                    <div className="text-right shrink-0">
                      ...
                      <p className="text-[9px] font-black text-ink-500 uppercase">
