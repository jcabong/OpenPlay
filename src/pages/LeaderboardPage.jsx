...
// ── For national/global: use player_elo table (global ELO) ───────────────────
async function fetchGlobalEloBoard({ sport }) {
...
// ── For court/city/province: calculate stats ONLY from games at that location ─
// This ensures Dasmarinas matches don't bleed into Parañaque rankings, etc.
async function fetchLocationBoard({ sport, scope, scopeValue }) {
  if (!scopeValue) return []

  const scopeField = scope === 'court' ? 'court_name' : scope === 'city' ? 'city' : 'province'

  // Fetch all tagged games at this specific location + sport
  const { data: games, error } = await supabase
    .from('games')
    .select('user_id, result, tagged_opponent_id')
    .eq('sport', sport)
    .eq(scopeField, scopeValue)
    .not('tagged_opponent_id', 'is', null)
    .or('is_deleted.is.null,is_deleted.eq.false')

  if (error) { console.error('games location fetch error:', error.message); return [] }
  if (!games?.length) return []

  // Tally wins/losses per user — ONLY from this location's games
  const statsMap = {}
  for (const g of games) {
    if (!statsMap[g.user_id]) statsMap[g.user_id] = { wins: 0, losses: 0 }
    if (g.result === 'win')  statsMap[g.user_id].wins++
    if (g.result === 'loss') statsMap[g.user_id].losses++
  }
  ...
  const { data: users, error: uErr } = await supabase
    .from('users')
    .select('id, username, display_name, city, province, avatar_url, avatar_type, elo_rating')
    .in('id', ids)
  ...
  return (users || [])
    .map(u => {
      const stats = statsMap[u.id] || { wins: 0, losses: 0 }
      ...
      return {
        ...
        city:         u.city         || '',
        province:     u.province     || '',
        // For location boards, rank by wins at this location (not global ELO)
        // We still show their global ELO as context, but sort by local wins
        elo:          u.elo_rating   || 1000,
        wins:         stats.wins,
        losses:       stats.losses,
        total,
        winRate:      total > 0 ? Math.round(stats.wins / total * 100) : 0,
        // localScore used for sorting: wins * 10 + winRate (tiebreaker)
        localScore:   stats.wins * 10 + (total > 0 ? Math.round(stats.wins / total * 100) : 0),
      }
    })
    // Sort by local wins (descending), tiebreak by win rate
...
export default function LeaderboardPage() {
  ...
  const loadBoard = useCallback(async () => {
    ...
    if (tier === 'national' || tier === 'global') {
      // Global ELO from player_elo table
      result = await fetchGlobalEloBoard({ sport })
    } else {
      // Location-scoped: calculate ONLY from games at that location
      const scopeValue = tier === 'court' ? selectedCourt
                       : tier === 'city'  ? selectedCity
                       : selectedProvince
      result = await fetchLocationBoard({ sport, scope: tier, scopeValue })
    }
  ...
  useEffect(() => {
    const ch = supabase.channel('lb-realtime')
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'games' },      () => loadBoard())
      .subscribe()
  ...
  // Label for the sort method shown to user
  const rankLabel = (tier === 'national' || tier === 'global')
    ? 'ELO Rating'
    : 'Local Wins'
    ...
    <div className="min-h-screen pb-24" style={{ background: '#0a0a0f' }}>
      ...
      <div className="px-5 pt-14 pb-4">
        ...
        <p className="text-[9px] font-black uppercase tracking-widest" style={{ color: C.accent, opacity: 0.8 }}>
          ELO Rating · Per Sport · Per Location
        </p>
      </div>
      ...
      {tier === 'court'    && (courtOptions.length    === 0
        ? <p className="text-xs font-bold text-center py-2 px-5" style={{ color: C.dim3 }}>No courts yet</p>
      ...
      {tier === 'city'     && (cityOptions.length     === 0
        ? <p className="text-xs font-bold text-center py-2 px-5" style={{ color: C.dim3 }}>No cities yet</p>
      ...
      {tier === 'province' && (provinceOptions.length === 0
        ? <p className="text-xs font-bold text-center py-2 px-5" style={{ color: C.dim3 }}>No provinces yet</p>
      ...
      <div className="px-5 mb-3 flex items-center gap-2">
        ...
        <div className="ml-auto flex items-center gap-1">
          ...
          <p className="text-[9px] font-black uppercase tracking-widest" style={{ color: C.dim2 }}>
            {rankLabel} · {SPORTS.find(s => s.id === sport)?.label}
          </p>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-16 px-6">
          ...
          <p className="text-xs mt-1" style={{ color: C.dim4 }}>
            {tier === 'national' || tier === 'global'
              ? 'Log a match with a tagged opponent to earn an ELO rating'
              : `No tagged matches recorded in ${contextLabel} yet`
            }
          </p>
        </div>
      ) : (
                  ...
                  <div key={player.id} className="flex items-center gap-3 p-4 border-b" style={{ borderColor: 'rgba(255,255,255,0.05)' }}>
                    ...
                    <div className="text-right shrink-0">
                      {(tier === 'national' || tier === 'global') ? (
                        ...
                        <>
                          <p className="font-display font-bold text-lg italic leading-none" style={{ color: C.accent }}>{player.wins}W</p>
                          <p className="text-[9px] font-black uppercase tracking-widest" style={{ color: 'rgba(200,255,0,0.6)' }}>{player.winRate}%</p>
                        </>
                      )}
