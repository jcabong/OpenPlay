...
// Fix: hard cap on per-page to avoid oversized queries
...
export default function FeedPage() {
  ...
  // Fix: guard against concurrent fetch calls
  ...
  // Fix: stable fetchFeed that doesn't include `page` in deps to avoid loops
  const fetchFeed = useCallback(async (isReset = true) => {
    if (fetchingRef.current && !isReset) return
    fetchingRef.current = true

    const from = isReset ? 0 : pageOffset
    ...
    if (isReset) {
      setPosts(newPosts)
      setPageOffset(POSTS_PER_PAGE)
      setLoading(false)
    } else {
      setPosts(prev => [...prev, ...newPosts])
      setPageOffset(prev => prev + POSTS_PER_PAGE)
      setLoadingMore(false)
    }

    setHasMore(newPosts.length === POSTS_PER_PAGE)
    fetchingRef.current = false
  }, [sport]) // Fix: only depend on sport, not pageOffset
      ...
      {loading ? (
        ...
        <div className="space-y-4">
          ...
          {!hasMore && posts.length > 0 && (
            <p className="text-center text-[10px] font-black uppercase tracking-widest py-6" style={{ color: 'rgba(255,255,255,0.2)' }}>
              You've seen it all 🏸
            </p>
          )}
        </div>
