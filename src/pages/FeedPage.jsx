...
export default function FeedPage() {
  ...
  // ── FIX: use refs for page tracking to avoid stale closure ──────────────
  const pageRef        = useRef(0)
  const loadingRef     = useRef(false)
  const hasMoreRef     = useRef(true)
  const sportRef       = useRef('all')
  ...
  // ── Core fetch function — uses refs, NOT stale state ──────────────────
  const fetchFeed = useCallback(async (reset = false) => {
    ...
    if (currentSport !== 'all') q = q.eq('sport', currentSport)

    const { data, error: fetchError } = await q

    if (fetchError) {
      console.error('Feed error:', fetchError.message)
    ...
    const newPosts = data || []
    const gotFull  = newPosts.length === POSTS_PER_PAGE
    ...
    if (reset) {
      ...
      setPosts(prev => {
        // Deduplicate by id in case of real-time overlaps
    ...
    loadingRef.current = false
  }, []) // ← no dependencies on state — all via refs

  // ── Sport change: update ref + reset ────────────────────────────────────
  useEffect(() => {
  ...
  }, [posts.length, fetchFeed])

  // ── Real-time subscription (targeted, not full refetch) ─────────────────
  useEffect(() => {
    const channel = supabase
      .channel('feed-realtime-v5')
      .on('postgres_changes', {
        event: 'INSERT',
        schema: 'public',
        table: 'posts',
      }, (payload) => {
        // Only prepend if it matches current sport filter
        if (sportRef.current === 'all' || payload.new?.sport === sportRef.current) {
          // Fetch just the new post with its relations
          supabase
            .from('posts')
            .select(`
              *,
      ...
      .on('postgres_changes', {
        ...
        // Update in-place rather than full refetch
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'likes' },
        () => fetchFeed(true)) // likes still do full reset (small cost)
  ...
  const handleTouchStart = (e) => {
    if (window.scrollY === 0 && !refreshing) setStartY(e.touches[0].clientY)
  }
    ...
    navigator.geolocation.getCurrentPosition(
      async ({ coords }) => {
        try {
          ...
          const place = data.address?.suburb || data.address?.city_district ||
                        data.address?.city || data.display_name
  ...
  async function handlePost() {
    ...
    try {
      ...
      resetComposer()
      // Don't full reset — real-time INSERT will prepend the new post
    } catch (err) {
    ...
    >
      ...
      {loading ? (
        ...
        <div className="text-center py-16 px-6">
          ...
          <p className="font-black uppercase text-xs tracking-widest mb-2" style={{ color: 'rgba(255,255,255,0.5)' }}>
            {error}
          </p>
          <button
            onClick={() => fetchFeed(true)}
            className="mt-4 px-5 py-2.5 rounded-xl text-xs font-black uppercase"
        ...
        <div className="text-center py-20 px-6">
          <p className="text-5xl mb-4">
            {sport !== 'all'
              ? SPORTS.find(s => s.id === sport)?.emoji || '🏸'
              : '🏸'
            }
          </p>
          <p className="font-black uppercase text-xs tracking-widest mb-2" style={{ color: 'rgba(255,255,255,0.4)' }}>
            {sport !== 'all'
              ? `No ${SPORTS.find(s => s.id === sport)?.label || sport} posts yet`
              : 'No posts yet'
            }
          </p>
          <p className="text-xs" style={{ color: 'rgba(255,255,255,0.2)' }}>
            {sport !== 'all'
              ? 'Be the first to post about this sport!'
          ...
          {sport !== 'all' && (
            <button
              ...
              className="mt-4 px-4 py-2 rounded-xl text-xs font-black uppercase border border-white/10"
        ...
        <div className="space-y-4">
          ...
          {loadingMore && (
            <div className="flex justify-center items-center gap-2 py-4">
              ...
              <span className="text-xs text-ink-500">Loading more...</span>
            </div>
          )}
