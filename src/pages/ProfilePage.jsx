...
import { MapPin, Edit2, Save, X, Loader2, Camera, Check, Trophy, MessageSquare, TrendingUp, TrendingDown } from 'lucide-react'
...
const TIERS = {
  ...
  Legend:   { color: '#c8ff00', bg: 'rgba(200,255,0,0.12)',   border: 'rgba(200,255,0,0.3)',   emoji: '👑' },
}

function TierBadge({ tier, elo, small = false }) {
  const cfg = TIERS[tier] || TIERS.Bronze
  return (
    <span
      className={`inline-flex items-center gap-1 font-black uppercase rounded-xl border ${small ? 'text-[9px] px-2 py-0.5' : 'text-[10px] px-2.5 py-1'}`}
      style={{ color: cfg.color, background: cfg.bg, borderColor: cfg.border }}
    >
      {cfg.emoji} {tier} {elo && <span className="font-bold opacity-80">· {elo}</span>}
    </span>
  )
}
...
function PostRow({ post }) {
  ...
  return (
    <div className="flex items-start gap-3 p-4 border-b border-white/5 last:border-none">
      ...
      <div className="flex-1 min-w-0">
        {sport && (
          <span className="text-[9px] font-black uppercase tracking-widest text-accent mb-1 block">{sport.label}</span>
        )}
        {post.content && (
          <p className="text-sm text-ink-200 leading-relaxed line-clamp-2">{post.content}</p>
        )}
        {hasMedia && (
          <p className="text-[9px] text-ink-600 font-bold mt-1">📎 {post.media_urls.length} media attached</p>
        )}
        {post.location_name && (
          <p className="text-[9px] text-ink-600 font-bold flex items-center gap-0.5 mt-0.5">
...
export default function ProfilePage() {
  ...
  const [playerElo, setPlayerElo]   = useState([])   // per-sport ELO rows
  const [eloHistory, setEloHistory] = useState([])   // recent ELO changes
  ...
  async function loadElo() {
    // Per-sport ELO from player_elo
    const { data: eloData } = await supabase
      .from('player_elo')
      .select('sport, elo_rating, wins, losses, matches_played, skill_tier')
      .eq('user_id', user.id)
      .order('elo_rating', { ascending: false })
    setPlayerElo(eloData || [])

    // Recent ELO history (last 10)
    const { data: histData } = await supabase
      .from('elo_history')
      .select('*, opponent:users!elo_history_opponent_id_fkey(username)')
      .eq('user_id', user.id)
      .order('created_at', { ascending: false })
      .limit(10)
    setEloHistory(histData || [])
  }
  ...
  async function handleSaveProfile() {
    ...
    const { error } = await supabase.from('users')
      .update({ display_name: formData.display_name, bio: formData.bio, city: formData.city, region: formData.region })
      .eq('id', user.id)
    if (!error) {
  ...
  return (
    <div className="min-h-screen bg-ink-900 text-ink-50 pb-28">
      ...
      {playerElo.length > 0 && (
        <div className="px-4 mb-5">
          <p className="text-[9px] font-black uppercase tracking-widest text-ink-600 mb-3 flex items-center gap-1.5">
            <Trophy size={11} className="text-accent" /> ELO per Sport
          </p>
          <div className="grid grid-cols-2 gap-2">
            {playerElo.map(pe => {
              const sport = SPORTS.find(s => s.id === pe.sport)
              const tier  = TIERS[pe.skill_tier] || TIERS.Bronze
              return (
                <div key={pe.sport} className="glass rounded-2xl p-3.5 border border-white/5"
                  style={{ borderColor: tier.border }}>
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-xl">{sport?.emoji || '🏸'}</span>
                    <TierBadge tier={pe.skill_tier} small />
                  </div>
                  <p className="font-display text-2xl font-bold italic" style={{ color: tier.color }}>{pe.elo_rating}</p>
                  <p className="text-[9px] font-black uppercase tracking-widest text-ink-600">{sport?.label}</p>
                  <p className="text-[9px] text-ink-600 mt-1">{pe.wins}W · {pe.losses}L · {pe.matches_played} played</p>
                </div>
              )
            })}
          </div>
        ...
        <div className="flex rounded-2xl overflow-hidden border border-white/10 glass">
          {TABS.map(tab => (
            <button key={tab} onClick={() => setActiveTab(tab)}
              ...
              {tab === 'Matches' ? <Trophy size={12} /> : tab === 'Posts' ? <MessageSquare size={12} /> : <TrendingUp size={12} />}
              {tab}
              {tab !== 'ELO' && (
        ...
        {activeTab === 'ELO' && (
          eloHistory.length === 0 ? (
            <div className="text-center py-14 text-ink-600 text-xs font-black uppercase tracking-widest">
              No ELO history yet — log a match with a tagged opponent!
            </div>
          ) : (
            <div className="glass rounded-[2rem] border border-white/10 overflow-hidden">
              {eloHistory.map((eh, i) => {
                const sport   = SPORTS.find(s => s.id === eh.sport)
                const isGain  = eh.rating_delta > 0
                const oppName = eh.opponent?.username || 'opponent'
                return (
                  <div key={i} className="flex items-center gap-3 p-4 border-b border-white/5 last:border-none">
                    <div className={`w-10 h-10 rounded-2xl flex items-center justify-center text-lg shrink-0 ${
                      isGain ? 'bg-accent/10' : 'bg-red-500/10'
                    }`}>
                      {sport?.emoji || '🏸'}
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-bold text-ink-100">{sport?.label}</p>
                      <p className="text-[9px] text-ink-500">vs @{oppName}</p>
                    </div>
                    <div className="text-right shrink-0">
                      <div className={`flex items-center gap-1 font-black text-sm ${isGain ? 'text-accent' : 'text-red-400'}`}>
                        {isGain ? <TrendingUp size={13} /> : <TrendingDown size={13} />}
                        {isGain ? '+' : ''}{eh.rating_delta}
                      </div>
                      <p className="text-[9px] text-ink-600">{eh.rating_before} → {eh.rating_after}</p>
                    </div>
                  </div>
                )
              })}
      ...
      {showAvatarPicker && (
        <AvatarPicker currentUrl={avatarUrl} currentType={avatarType}
          onSave={saveAvatar} onClose={() => setShowAvatarPicker(false)} />
      )}
    </div>
  )
}
