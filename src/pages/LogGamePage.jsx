...
// ── Opponent Search ─────────────────────────────────────────────────
// FIX: was querying non-existent 'profiles' table with wrong column 'full_name'
// Now correctly queries 'users' table with 'display_name'
function OpponentSearch({ taggedUser, searchQuery, onTag, onClear, onQueryChange }) {
  ...
  return (
    <div className="relative">
      ...
      {/* Dropdown — uses onMouseDown so blur doesn't close it first */}
      {focused && !taggedUser && results.length > 0 && (
        <div
          className="absolute z-50 w-full rounded-2xl overflow-hidden shadow-2xl border border-white/10"
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                {u.display_name && u.display_name !== u.username && (
                  <p className="text-xs truncate" style={{ color: 'rgba(200,255,0,0.7)' }}>
                    {u.display_name}
                  </p>
                )}
              </div>
...
export default function LogGamePage() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      console.log('✅ Your game recorded:', { id: game.id, result: game.result })

      // ── 2. Insert opponent's loss ──────────────────────────────────
      ...
      if (taggedUser && formData.result === 'win') {
        ...
        if (opponentError) {
          ...
          console.log('✅ Opponent loss recorded:', opponentGame?.id)
        }
      }
      ...
      const syncStatus = opponentGame
        ...
        : (taggedUser && formData.result === 'win'
            ? '⚠️ Opponent stats failed to update'
            : '')
