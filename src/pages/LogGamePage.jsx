...
export default function LogGamePage() {
  ...
  // Called by ShareGameModal when user taps "Post to Feed"
  const handlePostToFeed = useCallback(async ({ mediaUrl, caption, sport, locationName, gameId }) => {
    if (!user) return
    try {
      ...
      if (caption?.includes('@')) {
        await notifyMentions({
          text:     caption,
          fromUser: { id: user.id, username: profile?.username || 'user' },
        })
      }
    } catch (err) {
      console.error('Post to feed from modal error:', err)
    }
  }, [user, profile])
  ...
  return (
    <div className="min-h-screen pb-32" style={{ background: 'linear-gradient(160deg, #0a0a0f 0%, #0f1a0f 50%, #0a0a0f 100%)' }}>
      ...
      {/* Share Modal — opens after successful game submission */}
      {showShareModal && submittedGame && (
        <ShareGameModal
          game={submittedGame}
          profile={profile}
          onClose={() => {
